...
import { getCoinIcon } from './crypto-images'
...
const BLOCKCHAIN_MEME_TOKENS: Record<string, Array<{symbol: string, name: string, basePrice: number, contract?: string}>> = {
  solana: [
...
export class MemeCoinAPI {
  ...
  async getTopPairsByChain(chainId: string, limit: number = 20): Promise<CoinData[]> {
    try {
      const mappedChain = BLOCKCHAIN_MAPPING[chainId] || chainId
      
      // Use DexScreener API directly for accurate pricing
      const response = await fetch(`${this.baseUrl}/pairs/${mappedChain}?limit=${limit}`, {
        headers: {
  ...
  async searchPairs(query: string, chainId?: string): Promise<CoinData[]> {
    try {
      ...
      return this.transformPairsToCoins(data.pairs.slice(0, 20), chainId || 'solana')
    } catch (error) {
  ...
  private transformPairsToCoins(pairs: DexScreenerPair[], blockchain: string): CoinData[] {
    return pairs.map((pair, index) => ({
      ...
      makers: Math.floor(((pair.txns.h24?.buys || 0) + (pair.txns.h24?.sells || 0)) * 0.3), // Estimate makers
      changes: {
      ...
      blockchain,
      iconUrl: getCoinIcon(pair.baseToken.symbol),
      ...
      for (const [key, priceData] of allPrices) {
        ...
        if (chain === blockchain) {
          const coinData: CoinData = {
            id: tokenAddress,
            rank: blockchainPrices.length + 1,
            symbol: priceData.symbol,
            name: priceData.name || priceData.symbol,
            price: priceData.priceUsd,
            age: this.calculateAge(Date.now() - 24 * 60 * 60 * 1000), // Default to 1 day
            txns: Math.floor(priceData.volume24h * 100), // Estimate transactions from volume
            volume: priceData.volume24h / 1000000, // Convert to millions
            makers: Math.floor(priceData.volume24h * 50), // Estimate makers
            changes: {
              m5: priceData.priceChange5m,
              h1: priceData.priceChange1h,
              h6: priceData.priceChange24h / 4, // Estimate 6h from 24h
              h24: priceData.priceChange24h
            },
            liquidity: priceData.liquidity / 1000, // Convert to thousands
            mcap: priceData.marketCap / 1000000, // Convert to millions
            blockchain,
            iconUrl: getCoinIcon(priceData.symbol),
            isTopGainer: priceData.priceChange24h > 50,
            isNewPair: priceData.timestamp > Date.now() - 24 * 60 * 60 * 1000
          }
          
          blockchainPrices.push(coinData)
        }
      }
  ...
  private getFallbackData(blockchain: string, limit: number): CoinData[] {
    ...
    const blockchainTokens = BLOCKCHAIN_MEME_TOKENS[blockchain] || BLOCKCHAIN_MEME_TOKENS.solana
    ...
    return blockchainTokens.slice(0, limit).map((coin, index) => ({
      ...
      blockchain,
      iconUrl: getCoinIcon(coin.symbol),
